...
const DEPLOY_PATTERN =
  /^@r2-d2\s+(\S+)\s+(\S+)\s+(dev|stg|prd)\s+(\S+)\s+(\S+)\s+(https:\/\/api\.github\.com\/repos\/\S+)$/im;

// Maps app name → Makefile target prefix
const APP_TARGET_MAP: Record<string, string> = {
  triggr_api:       'api',
  frontend:         'fe',
  'patient-mobile': 'eas',
  'infra-cdk':      'cdk',
...
export default function deploy(robot: Robot): void {
  robot.hear(DEPLOY_PATTERN, async (msg: Response) => {
    ...
    if (AGORA_BOT_USER_ID && msg.message.user.id !== AGORA_BOT_USER_ID) return;
    ...
    msg.send(
      `:rocket: Deploying \`${app}/${bundle}\` (\`${tag}\`) → \`${env}\`\n` +
    ...
    exec(cmd, { timeout: 1_200_000, maxBuffer: 10 * 1024 * 1024 }, (err, stdout) => {
      if (err) {
        const tail = stdout.split('\n').slice(-30).join('\n');
        msg.send(
          `:x: Deploy FAILED: \`${app}/${bundle}\` (\`${tag}\`) → ${env}\n` +
          `\`\`\`\n${tail}\n\`\`\``,
        );
        return;
      }
      ...
      msg.send(
        `:white_check_mark: Deploy SUCCESS: \`${app}/${bundle}\` (\`${tag}\`) → ${env}\n` +
        `\`\`\`\n${recap}\n\`\`\``,
      );
    });
  });
